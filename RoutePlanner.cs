...
namespace DvMod.RemoteDispatch
{
    public static class RoutePlanner
    {
        ...
        {
            Reset();
            return new InvalidOperationException(message + " Recalculate the route.");
        }

        private static void RequireWorld()
        {
            if (!WorldStreamingInit.Instance || !WorldStreamingInit.IsLoaded)
                throw new InvalidOperationException("The world is not loaded.");
        }
        ...
        private static TrainCar GetCar(string guid)
        {
            ...
            if (car == null || !car.IsLoco) throw new ArgumentException("Locomotive unavailable.");
        ...
        private static IEnumerable<object?> CaptureRows(Topology result)
        {
            ...
            {
                var id = Id(track);
                if (result.native.ContainsKey(id)) throw new InvalidOperationException("Ambiguous track identifiers.");
        ...
        {
            if (!await requestGate.WaitAsync(0)) throw new InvalidOperationException("A route preview is already in progress. Try again.");
            try
            {
                return await Updater.RunOnMainThread<object>(() => {
                    ...
                    catch (Exception e) { warning = "AI status unavailable: " + e.Message; }
        ...
        public static async Task<object> Preview(string owner, string train, string destination, string? via)
        {
            ...
            try
            {
                ...
                var origin = await Updater.RunOnMainThread(() => {
                    ...
                    if (track == null) throw new InvalidOperationException("The train is not on a track.");
                    return Id(track);
                });
                var path = await Task.Run(() => t.graph.Find(origin, destination, via, null));
                return await Updater.RunOnMainThread<object>(() => {
                    if (topology != t) throw new InvalidOperationException("The network changed.");
                    ...
                    if (plans.Count >= 32) throw new InvalidOperationException("Too many active route previews.");
        ...
        private static List<string> Check(Plan plan, out bool isAi)
        {
            ...
            if (topology != t || RailTrackRegistry.Instance.OrderedJunctions != t.junctions)
                throw ChangedNetwork("The network changed.");
            var car = GetCar(plan.train);
            var conflicts = new List<string>();
            if (!ReferenceEquals(plan.multiplayerSession, MultiplayerData.SessionIdentity()))
                throw new InvalidOperationException("The multiplayer session changed. Recalculate the route.");
            ...
            for (int i = 1; i < plan.path.steps.Count; i++)
            {
                ...
                if (from == null || to == null) throw ChangedNetwork("A track disappeared.");
                ...
                if (next == null || !next.Any(b => b.track == to && b.first == step.forward) ||
                    ...
                    throw ChangedNetwork("Network connections changed.");
                if (step.junction >= 0 && (step.branch >= junction!.outBranches.Count ||
                    ...
                    throw ChangedNetwork("Switch branches changed.");
            }
            ...
            if (CurrentTrack(car) != t.native[plan.path.steps[0].track]) conflicts.Add("The train changed tracks. Recalculate the route.");
            if (Math.Abs(car.GetForwardSpeed()) > 0.1f) conflicts.Add("Stop the train before setting the route.");
            ...
            foreach (var step in plan.path.steps)
            {
                ...
                if (track == null) throw ChangedNetwork("A track disappeared.");
                if (occupied.TryGetValue(track, out var cars) && cars.Any(c => c != car && (car.trainset == null || c.trainset != car.trainset)))
                    conflicts.Add("Occupied track: " + step.track);
                if (reservations.TryGetValue(track, out var owner) && !ownCars.Contains(owner)) conflicts.Add("Track reserved by AITraffic: " + step.track);
                ...
                if (occupied.ContainsKey(junction.inBranch.track) || junction.outBranches.Any(b => occupied.ContainsKey(b.track)) ||
                    ...
                    conflicts.Add("J" + step.junction + ": an adjacent track is occupied (conservative protection).");
            }
        ...
        public static Task<object> ControlAi(string owner, string train, string action) => Updater.RunOnMainThread<object>(() => {
            if (!Main.settings.permissions.HasLocoControlPermission(owner)) throw new UnauthorizedAccessException("Locomotive control permission required.");
        ...
        public static Task<object> AssignAi(string owner, string token) => Updater.RunOnMainThread<object>(() => {
            if (!Main.settings.permissions.HasLocoControlPermission(owner) || !Main.settings.permissions.HasJunctionPermission(owner))
                throw new UnauthorizedAccessException("Locomotive and switch control permissions required.");
            ...
            plans.Remove(token);
            var conflicts = Check(plan, out var isAi);
            if (!isAi || plan.engineer == null) throw new InvalidOperationException("The driver changed. Recalculate the route.");
