...
class Program
{
    ...
    static void Main()
    {
        ...
        Refuses(()=>graph.Find("A","D",null,false), "No continuous route");
        Refuses(()=>graph.Find("A","E",null,null), "No continuous route");
        Refuses(()=>graph.Find("missing","D",null,null), "not found");
        ...
        Refuses(()=>graph.Find("A","D","C",null), "two positions");
        ...
        Refuses(()=>Await(RoutePlanner.Apply("other",token)),"expired");
        ...
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"expired");

        string Prepare() => Property<string>(Await(RoutePlanner.Preview("dispatcher","player","D",null)),"token");
        token=Prepare(); car.Speed=1;
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"Stop the train"); car.Speed=0;
        token=Prepare(); car.FrontBogie.track=b;
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"changed tracks"); car.FrontBogie.track=a;
        token=Prepare(); AiTrafficData.Reservations[d]="other";
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"reserved"); AiTrafficData.Reservations.Clear();
        ...
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"Occupied"); TrainCarRegistry.Instance.logicCarToTrainCar.Remove("other");
        j.selectedBranch=0; token=Prepare(); TrainCarRegistry.Instance.logicCarToTrainCar["other"]=new TrainCar("other",c);
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"adjacent"); TrainCarRegistry.Instance.logicCarToTrainCar.Remove("other");
        token=Prepare(); AiTrafficData.Engineers.Add("player");
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"driver assignment"); AiTrafficData.Engineers.Clear();
        ...
        Refuses(()=>Await(RoutePlanner.Apply("dispatcher",token)),"connections"); b.Out.Add(new Junction.Branch(d,true));
        token=Prepare();
