...
test('BDVM panel renders escaped authoritative state and submits bounded server intent', async () => {
  ...
  ['bdvmStatus','bdvmRefresh','bdvmFinances','bdvmFleet','bdvmMarket','bdvmLeases','bdvmAssignments'].forEach(element);
  const tab = element('tab'); const buttons = [];
  const state = { release:'1.0.0', authorityActor:'local', transportIdentity:'dispatcher', wallets:[{account:'Player:<unsafe>',Balance:10,Version:1}], fleet:[{AssetId:'a',DisplayName:'Loco <x>',kind:'Locomotive',state:'Available',owner:'Player:p',operatorRef:'Player:p'}], market:[], leases:[], assignments:[] };
  const calls = [];
